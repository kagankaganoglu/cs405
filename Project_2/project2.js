...
class MeshDrawer {
	...
	constructor() {
		...
		this.normalAttrLoc = gl.getAttribLocation(this.prog, 'normal');
	...
	draw(trans) {
		...
		// Set light position
		gl.uniform3f(this.lightPosLoc, lightX, lightY, 3);
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			 ...
			 */
			// Clamp coordinates outside the edge
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
			// Set minification filter for the texture
...
const meshFS = `
			...
			void main()
			{
				if(showTex && enableLighting){
					// Normalize the normal vector & add ambient and diffuse light:
					gl_FragColor = texture2D(tex, v_texCoord) * (ambient + dot(normalize(v_normal), -normalize(lightPos)));
				}
				else if(showTex){
